    ...
    def remove_nan(self, cloud_in):
        ...
        for point in cloud_in:
            if not math.isnan(point[0]) \
                    or not math.isnan(point[1]) \
                    or not math.isnan(point[2]):
                # object does not support item assignment use array to pc instead
                cloud_list.append(point)
        # TODO:resize point cloud not a good implement maybe resize() is better
        # not writeable.
        # cloud_out.size = j
        # point cloud need float32
        np_ndarray = np.asarray(cloud_list, dtype = np.float32)
        cloud_out.from_array(np_ndarray)
        return cloud_out

    def pc2_to_octree(self, point_cloud, resolution=0.002, point=Point(0.0, 0.0, 0.0)):
        ...
        for absolute_path, folders, files in os.walk(input_path):
            if file_name in files:
                with open(os.path.join(absolute_path, file_name), "r") as f_handle:
                    room = world_state_importer.xmltodict.parse(f_handle)
    ...
    def parse_room(world, dirname, files, class_lookup):
        ...
        if "room.xml" in files:
            with open(os.path.join(dirname, "room.xml"), "r") as f:
                room = world_state_importer.xmltodict.parse(f)
            room_name = room['SemanticRoom']["RoomStringId"]
